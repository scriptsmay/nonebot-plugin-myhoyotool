...
manually_game_sign = on_command(plugin_config.preference.command_start + '签到', rule=to_me(), priority=5, block=True)
...
async def _(event: MessageEvent, matcher: Matcher, command_arg=CommandArg()):
    """
    手动游戏签到函数
    """
    bot = get_bot()
...
manually_bbs_sign = on_command(plugin_config.preference.command_start + '任务',rule=to_me(), priority=5, block=True)
...
async def _(event: MessageEvent, matcher: Matcher, target: SaaTarget):
    """
    手动查看星穹铁道便笺（sr）
    """
    user_id = event.get_user_id()
...
async def perform_game_sign(
    ...
    for account in user.accounts.values():
        ...
        msgs_list = MessageFactory()
        """是否已经完成过签到"""
        game_record_status, records = await get_game_record(account)
        if not game_record_status:
            if matcher:
                msgs_list.append(Text(f"⚠️账户 {account.display_name} 获取游戏账号信息失败，请重新尝试"))
            continue
        games_has_record = []
        ...
        for class_type in BaseGameSign.available_game_signs:
            ...
            if not get_info_status:
                if matcher:
                    msgs_list.append(Text(f"⚠️账户 {account.display_name} 获取签到记录失败"))
            else:
                signed = info.is_sign
            ...
            if (get_info_status and not info.is_sign) or not get_info_status:
                ...
                if sign_status.need_verify:
                    if plugin_config.preference.geetest_url or user.geetest_url:
                        for i in range(3):
                            if matcher:
                                msgs_list.append(Text(f"⏳[验证码{i}] 正在尝试完成人机验证，请稍后..."))
                ...
                if not sign_status and (user.enable_notice or matcher):
                    ...
                    if matcher:
                        msgs_list.append(Text(message))
                    elif user.enable_notice:
                        await msgs_list.send_to(target=PlatformTarget.deserialize(target_dict))
            ...
            if user.enable_notice or matcher:
                ...
                msg_content = MessageFactory()
                if not get_info_status or not get_award_status:
                    # msg = f"⚠️账户 {account.display_name} 🎮『{signer.name}』获取签到结果失败！请手动前往米游社查看"
                    msg_content.append(Text(f"⚠️账户 {account.display_name} 🎮『{signer.name}』获取签到结果失败！请手动前往米游社查看"))
                else:
                    ...
                    if info.is_sign:
                        ...
                        # add img
                        msg_content.append(Image(img_file))
                        msg_content.send_to()
                    else:
...
async def perform_bbs_sign(
    ...
    for account in user.accounts.values():
        ...
        if msgs_list:
            MessageFactory(Text(msgs_list)).send()
...
manually_weibo_code_check = on_command(plugin_config.preference.command_start + 'wb兑换',rule=to_me(), priority=5, block=True)


@manually_weibo_code_check.handle()
async def weibo_code(event: MessageEvent, matcher: Matcher):
    user_id = event.get_user_id()
    user = PluginDataManager.plugin_data.users.get(user_id)
    await weibo_code_check(user=user, user_ids=[user_id], matcher=matcher)


manually_weibo_sign_check = on_command(plugin_config.preference.command_start + 'wb签到',rule=to_me(), priority=5, block=True)


@manually_weibo_sign_check.handle()
async def weibo_sign(event: MessageEvent, matcher: Matcher):
    user_id = event.get_user_id()
    user = PluginDataManager.plugin_data.users.get(user_id)
            ...
            if start:
                try:
                    for key, value in ticket_id.items():
                        ...
                        if matcher:
                            if img:
                                img_bytes = await get_file(img)
                                msg_content = MessageFactory([Image(img_bytes)])
                                
                            msg_content.append(Text(msg))
                            await msg_content.send()
...
def daily_update():
    ...
    每日图片生成函数
    """
    logger.info(f"{plugin_config.preference.log_head}后台开始daily_update")
...
@scheduler.scheduled_job("cron",
                         ...
                         id="daily_schedule")
async def daily_schedule(target_dict: Dict[str, Any]):
    """
    自动米游币任务、游戏签到函数
    """
