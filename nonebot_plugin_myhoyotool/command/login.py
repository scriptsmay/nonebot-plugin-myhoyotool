...
async def handle_first_receive(event: MessageEvent):
    ...
    if user_num <= plugin_config.preference.max_user or plugin_config.preference.max_user in [-1, 0]:
        ...
        if fetch_qrcode_ret:
            ...
            msg_content = MessageFactory([Text("请用米游社App扫描下面的二维码进行登录"), Image(image_bytes)])
            
            # if isinstance(event, MessageEvent):
            #     msg_content = OneBotV11MessageSegment.image(image_bytes)
            # else:
            #     await get_cookie.finish("⚠️发送二维码失败，无法登录")
            #     return
            try:
                await msg_content.send(reply=True)
                # await get_cookie.send(msg_img)
            except Exception as e:
                logger.exception("发送包含二维码的登录消息失败： {e}")
                await get_cookie.finish("⚠️发送二维码失败，无法登录")
...
async def _(event: MessageEvent, matcher: Matcher, bbs_uid=ArgStr()):
    """
